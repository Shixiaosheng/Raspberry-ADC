...
ADCreader::ADCreader()
{
  ...
  /// 0x40 is setup the ADC working in bipolar mode. 0x44 is unipolar mode.
  /// writeReg(fd,0x40);
  writeReg( fd, 0x44 );

  // tell the AD7705 that the next write will be to the gain register
  /// writeReg(fd,0x70);
  // the gain is 1 wrong. setup gain is in setup register.
  /// writeReg(fd,0x00);
